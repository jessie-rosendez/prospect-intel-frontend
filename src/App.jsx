...
const DATA_SOURCES = [
  { id: "aidentify", label: "Aidentify", desc: "Liquidity event intelligence feed", active: true },
  { id: "bloomberg", label: "Bloomberg Terminal", desc: "Real-time market & deal data", active: false },
  { id: "sec", label: "SEC Filings", desc: "Form 4, S-1, 13D/G filings", active: false },
  { id: "realestate", label: "Real Estate Records", desc: "Property transaction data", active: false },
  { id: "courtrecords", label: "Court Records", desc: "Probate, divorce, estate filings", active: false },
  { id: "manual", label: "Manual Entry", desc: "Direct event injection", active: true },
];
...
const styles = `
  ...
  *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
  :root {
    ...
    --signal-2: #9d8ff5;
    --signal-dim: #3d3470;
    --signal-glow: rgba(124,106,240,0.15);
    --signal-bg: rgba(124,106,240,0.06);
  ...
  .nav-item { display: flex; align-items: center; gap: 10px; padding: 9px 22px; font-family: var(--font-body); font-size: 13px; font-weight: 400; color: var(--dim); cursor: pointer; border: none; background: none; width: 100%; text-align: left; transition: color 0.12s, background 0.12s; border-left: 2px solid transparent; }
  .nav-item:hover:not(:disabled) { color: var(--soft); background: var(--ink-3); }
  .nav-item.active { color: var(--signal-2); border-left-color: var(--signal); background: var(--signal-bg); font-weight: 500; }
  .nav-item:disabled { opacity: 0.28; cursor: not-allowed; }
  .nav-icon { font-size: 13px; width: 16px; text-align: center; flex-shrink: 0; }
  .nav-badge { margin-left: auto; font-family: var(--font-mono); font-size: 8px; letter-spacing: 0.06em; padding: 2px 6px; border-radius: 10px; background: var(--signal-bg); border: 1px solid var(--signal-dim); color: var(--signal-2); }
  .sidebar-footer { padding: 14px 22px; border-top: 1px solid var(--rule); }
  .status-row { display: flex; align-items: center; gap: 8px; font-family: var(--font-mono); font-size: 9px; letter-spacing: 0.1em; color: var(--dim); text-transform: uppercase; }
  ...
  .main { display: flex; flex-direction: column; overflow: hidden; background: var(--ink); }
  ...
  .topbar-time { font-family: var(--font-mono); font-size: 10px; color: var(--muted); letter-spacing: 0.04em; }
  .content { flex: 1; overflow-y: auto; display: flex; flex-direction: column; }
  .content-padded { padding: 32px; }
  .content-full { height: 100%; }
  .welcome-page { display: flex; flex-direction: column; flex: 1; height: 100%; }
  .welcome-header { padding: 40px 48px 32px; border-bottom: 1px solid var(--rule); background: var(--ink-2); }
  .welcome-eyebrow { font-family: var(--font-mono); font-size: 9px; letter-spacing: 0.28em; text-transform: uppercase; color: var(--signal-dim); margin-bottom: 8px; }
  .welcome-heading { font-family: var(--font-display); font-size: 32px; font-weight: 600; color: var(--paper); letter-spacing: -0.02em; margin-bottom: 6px; }
  .welcome-sub { font-family: var(--font-body); font-size: 13px; color: var(--dim); line-height: 1.7; max-width: 560px; }
  .welcome-body { display: grid; grid-template-columns: 1fr 1fr; flex: 1; overflow-y: auto; }
  .welcome-col { padding: 36px 48px; }
  .welcome-col:first-child { border-right: 1px solid var(--rule); }
  .welcome-col-title { font-family: var(--font-mono); font-size: 9px; letter-spacing: 0.22em; text-transform: uppercase; color: var(--signal-dim); margin-bottom: 20px; display: flex; align-items: center; gap: 8px; }
  .welcome-col-title::after { content: ''; flex: 1; height: 1px; background: var(--rule); }
  .welcome-footer { padding: 20px 48px; border-top: 1px solid var(--rule); background: var(--ink-2); display: flex; align-items: center; justify-content: space-between; }
  .welcome-footer-hint { font-family: var(--font-mono); font-size: 10px; color: var(--muted); }
  .step-header { display: flex; align-items: center; gap: 12px; margin-bottom: 16px; }
  .step-num { width: 28px; height: 28px; border-radius: 50%; border: 1px solid var(--rule-2); display: flex; align-items: center; justify-content: center; font-family: var(--font-mono); font-size: 11px; color: var(--muted); flex-shrink: 0; }
  .step-num.done { border-color: var(--green); color: var(--green); background: var(--green-bg); }
  .step-num.active { border-color: var(--signal); color: var(--signal-2); background: var(--signal-bg); box-shadow: 0 0 12px var(--signal-glow); }
  .step-heading { font-family: var(--font-display); font-size: 17px; font-weight: 600; color: var(--paper); }
  .step-desc { font-family: var(--font-body); font-size: 12px; color: var(--dim); line-height: 1.6; margin-bottom: 18px; }
  .source-list { display: flex; flex-direction: column; gap: 8px; }
  .source-item { display: flex; align-items: center; gap: 14px; padding: 12px 14px; background: var(--ink-3); border: 1px solid var(--rule-2); border-radius: 3px; transition: border-color 0.12s; }
  .source-item.source-active { border-color: var(--signal-dim); background: var(--signal-bg); }
  .source-item.source-inactive { opacity: 0.5; }
  .source-dot { width: 8px; height: 8px; border-radius: 50%; flex-shrink: 0; }
  .source-dot.on { background: var(--signal); box-shadow: 0 0 6px var(--signal-glow); }
  .source-dot.off { background: var(--muted); }
  .source-info { flex: 1; }
  .source-name { font-family: var(--font-body); font-size: 13px; font-weight: 500; color: var(--soft); margin-bottom: 2px; }
  .source-desc { font-family: var(--font-mono); font-size: 9px; color: var(--dim); letter-spacing: 0.04em; }
  .source-badge { font-family: var(--font-mono); font-size: 8px; letter-spacing: 0.1em; text-transform: uppercase; padding: 2px 8px; border-radius: 2px; }
  .source-badge.live { color: var(--signal-2); background: var(--signal-bg); border: 1px solid var(--signal-dim); }
  .source-badge.soon { color: var(--muted); background: var(--ink-3); border: 1px solid var(--rule); }
  ...
  .prospect-location { font-family: var(--font-mono); font-size: 10px; color: var(--dim); margin-top: 2px; }
  ...
  .btn-primary { background: var(--signal); color: #fff; border: none; border-radius: 2px; font-family: var(--font-body); font-size: 13px; font-weight: 600; padding: 11px 22px; cursor: pointer; width: 100%; transition: opacity 0.12s, box-shadow 0.12s; letter-spacing: 0.01em; }
  .btn-primary:hover:not(:disabled) { opacity: 0.9; box-shadow: 0 0 16px var(--signal-glow); }
  ...
  .btn-inline { background: none; color: var(--signal-2); border: none; font-family: var(--font-mono); font-size: 10px; letter-spacing: 0.08em; cursor: pointer; padding: 0; transition: opacity 0.12s; }
  .btn-inline:hover { opacity: 0.7; }
  .style-result { background: var(--ink-3); border: 1px solid var(--rule-2); border-radius: 2px; padding: 16px; margin-top: 14px; }
  .style-result-label { font-family: var(--font-mono); font-size: 9px; letter-spacing: 0.16em; text-transform: uppercase; color: var(--muted); margin-bottom: 10px; }
  .style-chips { display: flex; flex-wrap: wrap; gap: 6px; margin-bottom: 8px; }
  ...
  .style-tone { font-family: var(--font-body); font-size: 12px; color: var(--mid); line-height: 1.6; }
...
function Sidebar({ page, onNavigate, apiStatus, setupDone }) {
  return (
    <div className="sidebar">
      ...
      <nav className="sidebar-nav">
        ...
        <button className={`nav-item ${page === "welcome" ? "active" : ""}`} onClick={() => onNavigate("welcome")}>
          <span className="nav-icon">{setupDone ? "✓" : "◎"}</span>
          Advisor Setup
          {setupDone && <span className="nav-badge">done</span>}
        </button>
        <div className="nav-section-label">Intelligence</div>
        <button className={`nav-item ${page === "prospects" ? "active" : ""}`} onClick={() => onNavigate("prospects")} disabled={!setupDone}>
          <span className="nav-icon">◈</span> Prospects
        </button>
        <button className={`nav-item ${page === "inject" ? "active" : ""}`} onClick={() => onNavigate("inject")} disabled={!setupDone}>
...
function TopBar({ page, selectedName, onBack }) {
  ...
  const labels = { welcome: "Advisor Setup", prospects: "Prospect Ledger", inject: "Event Injection" };
  return (
...
function WelcomePage({ onComplete, initialProfile }) {
  const [name, setName] = useState(initialProfile?.name || "");
  const [firm, setFirm] = useState(initialProfile?.firm || "");
  const [escalationEmail, setEscalationEmail] = useState(initialProfile?.escalationEmail || "");
  const [sample, setSample] = useState(initialProfile?.sample || "");
  const [styleResult, setStyleResult] = useState(initialProfile?.styleResult || null);
  const [styleLoading, setStyleLoading] = useState(false);
  const [styleError, setStyleError] = useState(null);
  const [profileSaved, setProfileSaved] = useState(!!initialProfile?.name);

  const handleSaveProfile = () => {
    if (!name.trim() || !firm.trim() || !escalationEmail.trim()) return;
    setProfileSaved(true);
  };

  const handleExtractStyle = async () => {
    if (!sample.trim()) return;
    setStyleLoading(true); setStyleError(null);
    try {
      const res = await api("/advisor/style", { method: "POST", body: JSON.stringify({ sample_text: sample.trim() }) });
      setStyleResult(res);
    } catch (e) { setStyleError(e.message); }
    finally { setStyleLoading(false); }
  };

  const canComplete = profileSaved && styleResult;

  return (
    <div className="welcome-page">
      <div className="welcome-header">
        <div className="welcome-eyebrow">Initial Configuration</div>
        <div className="welcome-heading">Configure Prospect Intel</div>
        <div className="welcome-sub">Complete your advisor profile and writing style setup before accessing the intelligence system. This ensures outreach is generated in your voice and escalations reach you correctly.</div>
      </div>
      <div className="welcome-body">
        <div className="welcome-col">
          <div className="step-header">
            <div className={`step-num ${profileSaved ? "done" : "active"}`}>{profileSaved ? "✓" : "1"}</div>
            <div className="step-heading">Advisor Profile</div>
          </div>
          <div className="step-desc">Your name, firm, and escalation routing. Hot prospect alerts will be sent to your escalation email immediately upon classification.</div>
          ...
          <div className="form-group">
            ...
            <input className="form-input" value={firm} onChange={e => setFirm(e.target.value)} placeholder="Firm name" />
          </div>
          <div className="form-group">
            <label className="form-label">Escalation Email</label>
            <input className="form-input" value={escalationEmail} onChange={e => setEscalationEmail(e.target.value)} placeholder="you@firm.com" />
          </div>
          {profileSaved ? (
            <div className="success-msg">✓ Profile saved — {name} at {firm}</div>
          ) : (
            <button className="btn-primary" disabled={!name.trim() || !firm.trim() || !escalationEmail.trim()} onClick={handleSaveProfile} style={{ marginBottom: 0 }}>
              Save Profile
            </button>
          )}
          <div style={{ marginTop: 32 }}>
            <div className="welcome-col-title">Data Sources</div>
            <div className="source-list">
              {DATA_SOURCES.map(src => (
                <div key={src.id} className={`source-item ${src.active ? "source-active" : "source-inactive"}`}>
                  <div className={`source-dot ${src.active ? "on" : "off"}`} />
                  <div className="source-info">
                    <div className="source-name">{src.label}</div>
                    <div className="source-desc">{src.desc}</div>
                  </div>
                  <span className={`source-badge ${src.active ? "live" : "soon"}`}>{src.active ? "Live" : "Soon"}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="welcome-col">
          <div className="step-header">
            <div className={`step-num ${styleResult ? "done" : profileSaved ? "active" : ""}`}>{styleResult ? "✓" : "2"}</div>
            <div className="step-heading">Outreach Style Profile</div>
          </div>
          <div className="step-desc">Paste a sample of your writing — an email, a LinkedIn message, any communication in your voice. The AI extracts your tone, formality, and characteristic phrases. All future outreach will be generated to match.</div>
          {styleError && <div className="error-msg">{styleError}</div>}
          <div className="form-group">
            <label className="form-label">Writing Sample</label>
            <textarea
              className="form-textarea"
              style={{ minHeight: 200, opacity: !profileSaved ? 0.4 : 1, pointerEvents: !profileSaved ? "none" : "auto" }}
              placeholder="Paste an email or message you've written..."
              value={sample}
              onChange={e => setSample(e.target.value)}
            />
          </div>
          {!styleResult ? (
            <button className="btn-primary" disabled={!sample.trim() || styleLoading || !profileSaved} onClick={handleExtractStyle}>
              {styleLoading ? "Extracting Style…" : "Extract Style Profile"}
            </button>
          ) : (
            <>
              <div className="style-result">
                <div className="style-result-label">Extracted Characteristics</div>
                <div className="style-chips">
                  {(styleResult.characteristics || []).map((c, i) => <span key={i} className="style-chip">{c}</span>)}
                </div>
                {styleResult.tone && <div className="style-tone">{styleResult.tone}</div>}
              </div>
              <div style={{ marginTop: 10 }}>
                <button className="btn-inline" onClick={() => setStyleResult(null)}>Re-extract style →</button>
              </div>
            </>
          )}
          <div style={{ marginTop: 28, padding: "16px", background: "var(--signal-bg)", border: "1px solid var(--signal-dim)", borderRadius: 3 }}>
            <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--signal-dim)", marginBottom: 8 }}>How this works</div>
            <div style={{ fontFamily: "var(--font-body)", fontSize: 12, color: "var(--dim)", lineHeight: 1.8 }}>
              Agent 1 reads your style profile before generating every outreach email. The extracted characteristics — tone, sentence length, formality, vocabulary — are injected into the prompt so the output matches your voice, not a generic AI template.
            </div>
          </div>
        </div>
      </div>
      <div className="welcome-footer">
        <div className="welcome-footer-hint">
          {!profileSaved ? "Complete advisor profile to continue" : !styleResult ? "Extract style profile to continue" : "All steps complete — ready to enter"}
        </div>
        <button
          className="btn-primary"
          style={{ width: "auto", padding: "11px 32px", opacity: canComplete ? 1 : 0.3 }}
          disabled={!canComplete}
          onClick={() => onComplete({ name, firm, escalationEmail, sample, styleResult })}
        >
          Enter Prospect Intel →
        </button>
      </div>
    </div>
  );
    ...
    <div className="content-padded">
      ...
      {!loading && !error && filtered.length > 0 && (
        <table className="prospects-table">
          ...
          <tbody>
            {filtered.map(p => (
              <tr key={p.id} onClick={() => onSelect(p.id, p.full_name)}>
                <td>
                  <div className="prospect-name">{p.full_name}</div>
                  <div className="prospect-location">{(p.location || "").split(".")[0]}</div>
                </td>
...
function ConversationPage({ prospectId, advisorProfile }) {
    ...
    <div className="conversation-layout content-full">
      <div className="conv-sidebar">
        <div className="prospect-card">
          <div className="prospect-card-name">{prospect.full_name}</div>
      ...
      <div className="conv-main">
        ...
        {/* Show stored reply from DB if it exists */}
        ...
        {/* Simulate reply input — only show if no reply yet */}
        {!hasExistingReply && (
          <div className="section-card">
            <div className="section-card-header">
              <span className="section-card-title">Reply Intelligence — Agent 2 + 3</span>
                ...
                <>
                  ...
                  <div className="agent-grid">
                    ...
                    <div className="agent-block">
                      ...
                      <div className="kv"><div className="kv-label">Auto Reply Safe</div><div className="agent-block-value" style={{ fontFamily: "var(--font-mono)", fontSize: 11 }}>{replyResult.policy_result?.auto_reply_safe ? "Yes" : "No"}</div></div>
                    </div>
                  </div>
...
function InjectPage({ onSuccess, advisorProfile }) {
  ...
  const samples = [
    { name: "Daniel Park", text: "Daniel Park, New York NY. Received $6.2M following IPO lockup expiration of his equity stake in a Series C fintech startup. Previously banked with JP Morgan private client. Relationship lapsed 18 months ago.", type: "IPO Liquidity" },
    { name: "Sofia Reyes", text: "Sofia Reyes, Miami FL. Sold minority stake in family-owned medical device distribution company for $2.8M. No current wealth manager. First-generation wealth.", type: "Business Exit" },
    { name: "Marcus Webb", text: "Marcus Webb, Dallas TX. Received $4.7M inheritance following death of mother. Recently retired from corporate law, age 58. No current advisor.", type: "Inheritance" },
  ];
          ...
          <div className="section-card">
            ...
            <div className="section-card-body">
              <div className="form-group">
                ...
                <textarea className="form-textarea" placeholder="Full name, location, event details, asset size..." value={rawText} onChange={e => setRawText(e.target.value)} style={{ minHeight: 140 }} />
              </div>
              <div className="two-col">
                <div className="form-group">
                ...
                <div className="form-group">
                  ...
                  <input className="form-input" value={toEmail} onChange={e => setToEmail(e.target.value)} />
                </div>
              </div>
...
export default function App() {
  ...
  useEffect(() => {
    API.health.check().then(() => setApiStatus("ok")).catch(() => setApiStatus("err"));
  }, []);
  ...
  const handleSetupComplete = (profile) => { setAdvisorProfile(profile); setSetupDone(true); setPage("prospects"); };
  ...
  return (
    <>
      <style>{styles}</style>
      <div className="shell">
        <Sidebar page={page} onNavigate={(p) => { handleBack(); setPage(p); }} apiStatus={apiStatus} setupDone={setupDone} />
        <div className="main">
          <TopBar page={page} selectedName={selectedName} onBack={handleBack} />
          <div className={`content ${showConversation ? "content-full" : ""}`}>
            {showConversation ? <ConversationPage prospectId={selectedId} advisorProfile={advisorProfile} /> :
              page === "welcome" ? <WelcomePage onComplete={handleSetupComplete} initialProfile={advisorProfile} /> :
              page === "prospects" ? <ProspectsPage onSelect={handleSelect} /> :
              page === "inject" ? <InjectPage onSuccess={handleSelect} advisorProfile={advisorProfile} /> :
              null}
          </div>
        </div>
      </div>
